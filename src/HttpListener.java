...
public class HttpListener implements Runnable {
    ...
    public void formtts() {
        this.send("HTTP/1.1 200 OK \r\n\r\n"
                ...
                + "</head>"
                + "<body>"
                + "<h1>Generate the audio file</h1>"
                + "<FORM method=get action='change_or_delete_tts'>"
                + "<TABLE BORDER=0><TR><TD>Current Message : </TD><TD>"
                + conf.getMessage_text() +
                "</TD></TR><TR><TD>New message : </TD><TD>"
                + "<INPUT type=text name='new'></TD></TR><TR><TD>"
    ...
    public HttpListener(Socket conn, ArrayList<String> status,conf_reader conf) {
        this.conn = conn;
        this.conf=conf;
        this.status = status;
    ...
    public void Homepage() {
        this.send("HTTP/1.1 200 OK \r\n\r\n"
                ...
                + "<A HREF=\"formtts.html\">Form TTS</A>"
                + "</body></html>\r\n");
    }
    ...
    public void run() {//thread deals with  the Http requests
        try {
            ...
            if ((request.toUpperCase()).startsWith("GET")) {
                ...
                if (url.startsWith("change_or_delete_tts")){
                    File f = new File(conf.getMessage_wav());
                    if (f.exists()){
                        f.delete();
                    }
                    String newmessage=url.substring(url.indexOf("new=")+4,url.indexOf("&change=Change"));
                    if (url.contains("&delete=Delete")|| newmessage.equals("")){
                        conf.setMessage_text("");
                        this.formtts();
                    }
                    else {
                        f.createNewFile();
                        conf.setMessage_text(newmessage);
                        Voice voice; FreeTTS freetts; voice =
                        VoiceManager.getInstance().getVoice("kevin16"); 
                        if (voice != null) {
                        voice.allocate(); }
                        freetts = new FreeTTS(voice);
                        String filename=conf.getMessage_wav().substring(0,conf.getDefault_message().indexOf("."));
                        ...
                        voice.speak(newmessage);
                        sfap.close();
                        this.formtts();                       
                    }
                }
                else if (url.equalsIgnoreCase("formtts.html")) {
                    this.formtts();
                } else if (url.equalsIgnoreCase("") || url.equalsIgnoreCase("index.html")) {
                    this.Homepage();
                            ...
                            if (timer == 0) {
                                ...
                                ActionListener taskPerformer = new ActionListener() {//actionListener to delay the delivery and form the status page
                                    ...
                                    public void actionPerformed(ActionEvent evt) {
                                        try {
                                            Boolean ok = SendEmail(from, to, subject, smtp, msg);
                                            if (ok) {
                                                ...
                                                String smtpsender = getSMTP(from);
                                                if (smtpsender != null) {
                                                    SendEmail(from, from, "unsuccessfull sending: " + subject, smtpsender, msg);
                                                }
